...
export async function compareModels(file: File, payload: ComparePayload): Promise<CompareResult> {
  ...
  if (USE_MOCK) return mockCompare(form, payload)

  const res = await fetch(`${API_URL}/api/compare`, { method: 'POST', body: form })
  if (!res.ok) {
    const msg = await res.text().catch(() => '')
    throw new Error(`Compare failed: ${res.status} ${res.statusText} ${msg}`)
  }
